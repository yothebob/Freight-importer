...
# find file in file explorer to import
def set_file(_file):
    global file
    file= _file
...
def import_excel_estimate():
    ...
    #search areas for any item that is fulfilled
    for area in areas:
...
def load_in_data(_dict):
    ...
    for num in range(3,109):
        if freight_wb['B'+str(num)].value in _dict.keys():
            freight_wb['D'+str(num)] = _dict[freight_wb['B'+str(num)].value]
            
    #name and save new freight file
    freight_name=input('what do you want to call your freight quote?: ')
    freight.save(filename=(freight_name+'.xlsm'))


def estimate_bundles(_dict):
    ...
    #build bundles, pallets and boxes based off part type/ quantity
...
def run_freight_importer():
    data = import_excel_estimate()
    fulfill = load_in_data(data)
    palletize = estimate_bundles(data)

window = Tk()
window.title('Freight Importer')
window.geometry('300x100')
open_button = Button(master=window,width=20,text='Open File',command=file_name)
import_button = Button(master=window,width=20,text='Transfer Data',command=run_freight_importer)
open_button.pack()
import_button.pack()
window.mainloop()
